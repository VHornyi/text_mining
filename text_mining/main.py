from utils import load_races, load_lotr_texts, preprocess_text # create_ngrams jest importowane w analysis.py
from analysis import (
    analyze_word_frequencies,
    analyze_ngrams,
    build_dtm_tfidf, # Ta funkcja jest teraz zmodyfikowana
...
def main():
    ...
    doc_names = list(lotr_texts_dict.keys()) # doc_names już tu jest
    ...
    # Przekazujemy doc_names do build_dtm_tfidf
    ...
    if dtm is not None and dtm_feature_names is not None:
        ...
        if actual_lda_topics > 0: # Upewniamy się, że liczba tematów jest dodatnia
    ...
    if tfidf_matrix is not None and tfidf_feature_names is not None:
        if tfidf_matrix.shape[0] > 0 and tfidf_matrix.shape[1] > 0:
            # n_topics jest dostosowywane wewnątrz funkcji topic_modeling_lsa
    ...
    if tfidf_matrix is not None and doc_names:
        ...
        # n_clusters nie może być większe niż liczba dokumentów
        actual_n_clusters = min(3, num_docs) if num_docs > 0 else 0
        if actual_n_clusters > 0 : # K-Means wymaga co najmniej 1 klastra (chociaż sensowne są co najmniej 2)
             # Jeśli actual_n_clusters = 1, to wszystkie dokumenty będą w jednym klastrze
